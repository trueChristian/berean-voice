...
- Treat publisher-added layout treatments—including separate article or poem text, translucent reading panels, opacity fades or colour washes, section/header/footer bars, page rules and numbers, decorative frames or shadows, and text banners—as layout overlays rather than image content. Do not bake a separable layout overlay into an exported photograph or illustration.
- For every layered, tiled, masked, or full-page background image, inspect the raw PDF image objects, paint order, clipping paths, soft masks, and transparency groups as well as the rendered page. Export the clean base asset when one exists; otherwise reassemble only the exact base-image pieces at their PDF coordinates. A page render with text objects suppressed is not proof that the remaining raster is overlay-free.
- Omit separable layout overlays while preserving the underlying source pixels, published crop, aspect ratio, orientation, and useful resolution. Do not mistake intrinsic fog, depth-of-field blur, lighting, toning, or texture for an overlay. Never generate, inpaint, clone, heal, or otherwise synthesize pixels hidden by an overlay. A uniform flattened colour or opacity blend may be reversed only when the exact blend is established by retained source black/white points or equivalent object evidence; record the evidence and deterministic inverse transformation. If an overlay is otherwise irreversibly baked into the only source raster, use a meaningful unobstructed crop only when it remains faithful and useful; otherwise retain the exact source raster and document the unavoidable overlay explicitly instead of claiming that the asset is clean.
- In each affected image's `rights_note` or verification metadata, identify the clean base object or reconstruction used and name the omitted overlay treatments. Preserve text or markings intrinsic to the only source raster or substantive to a standalone chart, diagram, or graphic; separately painted publisher typography remains a removable overlay.
...
21. Review the final diff and repository status.
